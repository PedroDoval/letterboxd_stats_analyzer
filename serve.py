...
def upload_file():
    try:
        ...
        if uploaded_file.filename != "":
            ...
            # For all history
            diary_full = data_processor.get_diary(
                os.path.join(config["input_dir"], "diary.csv"), year=None
            )
            ...
            # For all years
            diary_year = data_processor.get_diary(
                os.path.join(input_dir, "diary.csv"), year
            )
            ...
            (
                ...
                avg_rate,
            ) = data_processor.analyze_ratings_distribution(diary_year)
            ...
            month_distribution_data = (
                data_processor.analyze_distribution_films_by_month(diary_year, year)
            )
            plotter.plot_distribution_films_by_month(month_distribution_data)

            week_distribution_data = data_processor.analyze_distribution_films_by_week(
                diary_year, year
            )
            plotter.plot_distribution_films_by_week(week_distribution_data)

            rewatched_rate, rewatched_percentage = data_processor.get_rewatched_rate(
                diary_year
            )
        ...
        return render_template(
            "accepted.html", plot_files=plot_files, user=profile["Username"], year=year
        )
