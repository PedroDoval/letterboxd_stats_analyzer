...
def get_weekday_distribution(diary):
    ...
    counter_weekday = dict(
        OrderedDict(
            sorted(
                counter_weekday.items(),
                key=lambda x: utils.weekdays_order_en.index(x[0]),
            )
        )
    )
...
def get_month_distribution_all_year(diary):
    group_months = (
        diary.groupby(["month", "year"]).size().reset_index(name="count_movies")
    )
    group_months["unique_month"] = group_months.apply(
...
def get_month_distribution(diary):
    ...
    counter_months = dict(
        OrderedDict(
            sorted(
                counter_months.items(), key=lambda x: utils.months_order_en.index(x[0])
            )
        )
    )
...
def analyze_distribution_films_by_month(diary, year=None):
    month_distribution_data = (
        get_month_distribution_all_year(diary)
        if year == None
        else get_month_distribution(diary)
    )
...
def analyze_distribution_films_by_week(diary, year=None):
    week_distribution_data = (
        get_weekday_distribution(diary)
        if year == None
        else get_weekday_distribution(diary)
    )
