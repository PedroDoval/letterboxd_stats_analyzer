...
def plot(config_file):
    ...
    diary_year = data_processor.get_diary(
        os.path.join(config["input_dir"], "diary.csv"), year
    )
    ...
    ratings_distribution, avg_rate = data_processor.analyze_ratings_distribution(
        diary_year
    )
    ...
    month_distribution_data = data_processor.analyze_distribution_films_by_month(
        diary_year, year
    )
