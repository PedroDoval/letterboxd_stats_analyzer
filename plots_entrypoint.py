...
class Plotter():
    ...
    def plot_distribution_films_by_month(self, month_distribution_data):
        ...
        plot_utils.plot_bars_messages(month_distribution_data, title, self.output_dir, "Mes", filename, bg_transparent=self.web_service)
    ...
    def plot_distribution_films_by_week(self, week_distribution_data):
        ...
        plot_utils.plot_bars_messages(week_distribution_data, title, self.output_dir, "Dia de la semana", filename, bg_transparent=self.web_service)

    def plot_distribution_films_by_year(self, year_distribution):
        title = "Películas cada año"
        filename = "films_by_year"
        plot_utils.plot_bars_messages(year_distribution, title, self.output_dir, "Año", filename, bg_transparent=self.web_service)
    ...
    def plot_ratings_entrypoint(self, ratings_data, diary):
        ...
        plot_utils.plot_table(max_films, max_rate, min_films, min_rate, self.output_dir, filename, bg_transparent=self.web_service)
    ...
    def plot_ratings_distribution_entrypoint(self, ratings_distribution, avg_rate):
        ...
        plot_utils.plot_bars_messages(ratings_distribution, title, self.output_dir, "Votación", filename, bg_transparent=self.web_service)

    def plot_reviews_wordcloud(self, texts, config):
        plot_utils.wordcloud(texts, config["output_dir"], bg_transparent=self.web_service)
