...
def plot_bars_messages(count_messages, title, output_dir, filename=None):
    ...
    fig.savefig(os.path.join(output_dir, filename + ".png"), transparent=True)
    #plt.show()
...
def plot_day_and_hour(df, title, user, output_dir, filename=None):
    ...
    fig.savefig(os.path.join(output_dir, "plot_day_and_hour_" + user + ".png"), transparent=True)
...
def plot_pie_messages(count, title, output_dir, filename=None):
    ...
    fig = plt.figure()
    plt.title(title)
    plt.pie(values, labels=names, autopct=lambda p: '{:.0f}'.format(p * sum(values) / 100), shadow=True)
    fig.savefig(os.path.join(output_dir, filename + ".png"), transparent=True)
    #plt.show()
...
def plot_lists_pies(lists, config, filename=None):
    ...
    fig = plt.figure()
    fig.suptitle('Listas del usuario')
    # 2 rows 2 columns
    ...
    fig.savefig(os.path.join(config["output_dir"], filename + ".png"), transparent=True)
    #plt.show()
